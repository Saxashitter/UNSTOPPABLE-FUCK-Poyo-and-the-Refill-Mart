...
local toucheries = {} -- modding this rq
...
function lovepad:update()
    ...
    for _, button in pairs(self.buttons) do
        ...
        for _, touch in ipairs(touches) do
        	...
        	if not toucheries[touch] then
        		toucheries[touch] = {xt = xt, yt = yt}
        	end
        	
        	local ogxt, ogyt = toucheries[touch].xt, toucheries[touch].yt
            if (math.abs((xt - button.x))^2 + math.abs((yt - button.y))^2)^0.5 < button.radius
            or (math.abs((ogxt - button.x))^2 + math.abs((ogyt - button.y))^2)^0.5 < button.radius then
                button.isDown = true
            end
        end
    end
...
function lovepad:touchreleased(id)
	toucheries[id] = nil
end
